...
fn main(){
    ...
    add.insert(num3);
    if let Some(maior) = add.iter().max(){  //Itera sobre os valores dentro do hashset e retorna o maior
        println!("O maior valor digititado foi: {}", maior);
    }
    if let Some(menor) = add.iter().min(){ //Itera sobre os valores e retorna o menor
        println!("O menor valor digititado foi: {}", menor);
    }
}